...
// custom properties for display control
bool isSSAO;
bool isFXAA;
...
int main()
{
    ...
    glfwSetInputMode(window, GLFW_STICKY_KEYS, .3f);
    ...
    Shader shaderFXAA("ssao.vs", "fxaa.fs");
    ...
    // also create framebuffer to hold FXAA processing stage
    // -----------------------------------------------------
    unsigned int fxaaFBO;
    glGenBuffers(1,&fxaaFBO);
    glBindFramebuffer(GL_FRAMEBUFFER, fxaaFBO);
    unsigned int fxaaBuffer;
    ...
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
        std::cout << "FXAA Framebuffer not complete!" << std::endl;
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
    ...
    shaderFXAA.setVec2("inverseScreenSize", glm::vec2(1.0/SCR_WIDTH, 1.0/SCR_HEIGHT));
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        shaderSSAO.setBool("isActive", isSSAO);
        ...
        if (isFXAA) { //Nawak
          ...
          //glClear(GL_COLOR_BUFFER_BIT);
          shaderFXAA.use();
          shaderFXAA.setVec2("inverseScreenSize", glm::vec2(SCR_WIDTH, SCR_HEIGHT));
